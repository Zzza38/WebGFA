...
const db = (await import(`../data/database.json`, { with: { type: "json" } })).default;
...
const HTTP_PORT = Number(process.env.PORT ? process.env.PORT : dev ? webgfaConfig.ports.development : webgfaConfig.ports.main);
...
if (webgfaConfig.installed.interstellar) {
    ...
    games.tools["Interstellar"] = webgfaConfig.features.interstellarURL;
}
...
//                     FUNCTIONS                           //
/////////////////////////////////////////////////////////////
async function handleMainRequest(request, reply) {
    const reqPath = normalizePath(request.url.split("?")[0]);
    const sessionId = request.cookies.uid;
    if (!sessionId)
        return console.error("No session ID on a user. Please open a GitHub issue, this shouldn't be possible");
    ...
    if (isHtmlRequest(reqPath)) {
        ...
        await handleStatistics(request, reply, user, sessionId); // TODO: actually fix it
    }
}
async function handleLogin(request, reply) {
    const { username, password } = request.body;
    if (!db.users[username])
        return reply.code(403).send("User doesn't exist");
...
async function handleGuestSession(request, reply) {
    const sessionID = String(request.cookies?.uid || "NO-SESSION-ID");
    const isGuest = sessionID.includes("GUEST-ACCOUNT-");
    const isUser = Object.values(db.users).some((user) => user.sessionID === sessionID);
    if (isGuest || isUser)
        return; // already fine, continue chain
...
async function handleGitHubWebhook(request, reply) {
    reply.code(202).send("Accepted");
    if (webgfaConfig.features.githubAutoPull.pull === false)
        return;
...
async function handleStatistics(request, reply, user, sessionID) {
    const reqPath = normalizePath(request.url.split("?")[0]);
    ...
    if (!db.gamePopularity.updated) {
        ...
        if (db.gamePopularity.updated.month !== getStartOfMonth().toISOString()) {
            ...
            for (const [key, value] of Object.entries(db.gamePopularity)) {
                if (key === "updated" || typeof value !== "object" || value === null)
                    continue;
                if ("monthly" in value)
                    value.monthly = 0;
            }
    ...
    if (reqPath.includes("/games")) {
        ...
        let gamePop = (existingPop && 'allTime' in existingPop) ? existingPop : {
            allTime: 0,
            monthly: 0,
            weekly: 0,
            url: url,
...
let csvWriteLock = Promise.resolve();
...
async function writeJSONChanges(json, JSONpath = "../data/database.json") {
    const currentWrite = writeLock.then(async () => {
        try {
            await fs.promises.writeFile(path.resolve(__dirname, JSONpath), JSON.stringify(json, null, 2), 'utf8');
        }
        catch (error) {
...
async function handleApiRequest(request, reply) {
    try {
        // Extract 'service' from the URL
        let service = request.url.split("?")[0].split("/")[2];
        if (!service)
            return reply.code(400).send('Missing service parameter');
        ...
        if (!user)
            return reply.code(401).send('User does not exist');
        ...
        const postHandler = {
            ...
            'reset-password': async () => {
                const { username, resetID, password } = request.body;
                // Validate required fields
            ...
            'webgpt': async () => {
                ...
                try {
                    ...
                    const personalities = webgfaConfig.features.openai.personalities || {};
                    // Get slang dictionary from config
                    const slangDictionary = webgfaConfig.features.openai.slangDictionary || {};
                    ...
                    if (personality && personalities[personality]) {
                        systemPrompt = `${personalities[personality]} ${systemPrompt}`;
                    ...
                    if (!db.users[user].coinHistory)
                        db.users[user].coinHistory = [];
                    ...
                    const refusalMessage = completion.choices[0].message.refusal;
            ...
            'add-coins': async () => {
                ...
                const { targetUser, amount, reason } = request.body;
                ...
                // Record transaction
                if (!db.users[targetUser].coinHistory)
                    db.users[targetUser].coinHistory = [];
            ...
            'admin-create-user': async () => {
                ...
                const { username, password, permissions, email, coins } = request.body;
                // Validation
            ...
            'admin-delete-user': async () => {
                ...
                const { username: targetUser } = request.body;
                // Validation
                if (!targetUser) {
            ...
            'admin-update-permissions': async () => {
                ...
                const { username: targetUser, permissions } = request.body;
                // Validation
            ...
            'admin-reset-password': async () => {
                ...
                const { username: targetUser, newPassword } = request.body;
                // Validation
            ...
            'admin-update-user-save': async () => {
                ...
                const { username: targetUser, save } = request.body;
                // Validation
            ...
            'admin-reset-game-stats': async () => {
                ...
                const { gameName, resetType } = request.body;
                // Validation
                ...
                const gameStats = db.gamePopularity[gameName];
                // Reset based on type
        ...
        const getHandler = {
            ...
            'getTools': async () => {
                let base = games.tools || {};
                // Filter out Admin Panel for non-admin users
                if (!db.users[user].permissions?.includes('admin')) {
                    base = Object.fromEntries(Object.entries(base).filter(([key]) => key !== 'Admin Panel'));
                }
                const prem = db.users[user].permissions?.includes('prem')
                    ? games.premiumTools || {}
                    : {};
            ...
            'is-premium': async () => {
                if (user === "guest")
                    return reply.send({ premium: false });
            ...
            'admin-get-user-details': async () => {
                ...
                const targetUser = String(request.query.username || '');
                if (!targetUser) {
        ...
        if (request.method === 'POST' && postHandler) {
            ...
            if (dev)
                console.info(`Invalid service: ${service}`);
...
function serveHtmlFile(reqPath, request, reply) {
...
let oldTable = [];
async function updateTable(jsonObject, filePath, oldTable = []) {
    let table = oldTable || [];
