...
(async () => {
    ...
    if (newConfig.features.interstellar)
        newConfig.features.interstellarURL = await prompt(`Interstellar URL: `, config.features.interstellarURL);
    if (newConfig.features.webssh)
        newConfig.features.websshURL = await prompt(`WebSSH URL: `, config.features.websshURL);
    ...
    if (newConfig.features.githubAutoPull.pull)
        newConfig.features.githubAutoPull.restartCommand = await prompt(`Restart command for after a GitHub pull (Default is ${config.features.githubAutoPull.restartCommand}): `, config.features.githubAutoPull.restartCommand);
    ...
    if (newConfig.features.login.enabled)
        newConfig.features.login.emailEnabled = (await prompt(`Email enabled for logging in? (Default is ${config.features.login.emailEnabled ? "Y" : "N"}): `, config.features.login.emailEnabled ? "Y" : "N")).toLowerCase().includes("y");
    if (newConfig.features.login.emailEnabled)
        newConfig.features.login.email = await prompt(`Email for logging in (Default is ${config.features.login.email}): `, config.features.login.email);
    if (newConfig.features.login.enabled)
        newConfig.features.login.url = await prompt(`Main URL: `, config.features.login.url);
    ...
    if (newConfig['client-config'].email.enabled)
        newConfig['client-config'].email.email = newConfig.features.login.email;
